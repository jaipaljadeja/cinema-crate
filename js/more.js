...
function searchKeyword(cat, type) {
    document.querySelector('#movie-results').innerHTML = "";
    document.querySelector('#tv-results').innerHTML = "";
    if (type == "movie") {
        const moviePath = `/movie/${cat}`;
        const movieSearchUrl = generateTmdbUrl(moviePath) + '&page=' + mCurrPage;
        itemsGrid(movieSearchUrl, "#movie-results", "Movie");
        document.querySelector('#tv-section').style.display = "none";
        document.getElementById('movie-cat-value').innerHTML = `| ${cat.toUpperCase()}`;
    }
    if (type == "tv") {
        const tvPath = `/tv/${cat}`;
        const tvSearchUrl = generateTmdbUrl(tvPath) + '&page=' + tCurrPage;
        itemsGrid(tvSearchUrl, "#tv-results", "TV");
        document.querySelector('#movie-section').style.display = "none";
        document.getElementById('tv-cat-value').innerHTML = `| ${cat.toUpperCase()}`;
    }
}
...
function changeMoviePage(page) {
    ...
    document.querySelector('#movie-results').innerHTML = "";
    const moviePath = `/movie/${cat}`;
    const movieSearchUrl = generateTmdbUrl(moviePath) + '&page=' + page;
...
function changeTvPage(page) {
    ...
    document.querySelector('#tv-results').innerHTML = "";
    const tvPath = `/tv/${cat}`;
    const tvSearchUrl = generateTmdbUrl(tvPath) + '&page=' + page;
    itemsGrid(tvSearchUrl, "#tv-results", "TV");
}
